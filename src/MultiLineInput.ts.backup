...
export default function MultiLineInput({
  ...
  const getCursorLocation = useCallback((pos: number): CursorLocation => {
  ...
  const deleteChar = useCallback((direction: 'backward' | 'forward' = 'backward'): void => {
  ...
  const moveCursor = useCallback((newPos: number): void => {
